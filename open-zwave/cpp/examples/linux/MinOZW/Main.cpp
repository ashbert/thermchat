...
int main( int argc, char* argv[] )
{
	...
	// XXX: Needs all the various devices config XML thingy. Find a better way to provide this.
	Options::Create( "/opt/github/open-zwave/config/", "", "" );
	...
	string newcoolval = "72";
	int gettempval = 0;
	int c;	

	while ((c = getopt (argc, argv, "d:gs:")) != -1)
		switch (c)
		{
			case 'd':
				port = optarg;
	...
	if (!g_initFailed)
	{
		...
		{
			NodeInfo* nodeInfo = *it;
			// Get these mysterious values for nodeId and ClassId from the xml file that WriteConfig generates.
			// Needs a "training" run to get this right.
			if (nodeInfo->m_nodeId != 2) 
				...
				{
					ValueID v = *it2;
					if (v.GetCommandClassId() == 67)
					{
						//printf("Found SETPOINT classid, index=%d\n", v.GetIndex());
						if (v.GetIndex() == 2) {
							string cval;
							// Get prev value of temp.
							// XXX: For some reason GetValueAsInt doesn't work as expected.
							Manager::Get()->GetValueAsString(v, &cval);
							//printf("str Found cooling Setpoint: %s\n", cval.c_str());

							if (gettempval == 1) {
								printf("{\"Zwavestr\":\"CurrentTemp\",\"Temp\":%s}\n", cval.c_str());
								break;	
							}

							// Set new value of temp.
							Manager::Get()->SetValue(v, newcoolval);
							Manager::Get()->RefreshValue(v);

							// Check if the value takes.
							// XXX: TODO: Hook into the OnNotification handler to get nodeId changes.
							// Otherwise it'll just return the stale value.
							// Manager::Get()->GetValueAsString(v, &cval);
							// printf("NewCurrentTemp: %s\n", cval.c_str());
							// For now, trust me it works ;)
						
							printf("{\"Zwavestr\": \"NewCurrentTemp\" , \"Temp\": %s}\n", newcoolval.c_str());	
							break;
						}
						continue;
					}
